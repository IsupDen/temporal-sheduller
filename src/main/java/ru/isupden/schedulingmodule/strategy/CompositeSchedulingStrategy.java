...
public class CompositeSchedulingStrategy
    ...
    /* ---- compare ---- */
    @Override public boolean canCompare(Task a, Task b){ return true; }

    @Override
    public int compare(Task a, Task b) {
        for (var s: chain){
            if (!s.canCompare(a,b)) continue;
            int c = s.compare(a,b);
            if (c!=0) return c;
        }
        return 0;
    ...
    /* ---- delegation ---- */
    @Override public void preprocess(Queue<Task> q, Instant now){
        chain.forEach(s->s.preprocess(q,now));
    }
    @Override public void onDispatch(Task t, Instant at){
    ...
    /* ---- UsageAware ---- */
    @Override public void recordUsage(String tenant,double cost,Instant at){
        chain.stream()
