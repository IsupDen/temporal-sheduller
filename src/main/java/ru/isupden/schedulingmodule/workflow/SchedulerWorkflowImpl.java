...
public class SchedulerWorkflowImpl implements SchedulerWorkflow {
    ...
    public SchedulerWorkflowImpl(SchedulingModuleProperties p,
                                 ...
                                 DispatchActivity a) {
        this.props = p;  this.strategies = reg;  this.dispatch = a;
    }
        ...
        try {
            while (true) {
                ...
                Task next = ready.stream()
                        ...
                        .orElse(null);
                if (next == null) { Workflow.await(() -> !ready.isEmpty()); continue; }
                ready.remove(next);
    ...
    /* ───── signals / query ───── */
    @Override public void submitTasks(List<Task> tasks){ ready.addAll(tasks); }

    @Override public void reportUsage(String tenant, double cost) {
        if (strategy instanceof UsageAwareStrategy u)
            u.recordUsage(tenant, cost, now());
    }

    @Override public int getQueueLength(){ return ready.size(); }
    ...
    private SchedulingStrategy buildStrategy(String name){
        ...
        return list.size()==1 ? list.get(0) : new CompositeSchedulingStrategy(list);
    }

    private Instant now(){ return Instant.ofEpochMilli(Workflow.currentTimeMillis()); }

    /* запись метки времени успешного dispatch-а */
    private void recordDispatch(){ window.addLast(now()); }
